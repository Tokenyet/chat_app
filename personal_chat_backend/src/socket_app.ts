...
export class SocketApp {
  ...
  setup() {
    ...
    this.server.on('connection', async (socket) => {
      ...
      // give room info, this will trigger online ack
      ...
      socket.on(
        ...
        (ids: string[], callback: (onlines: string[]) => void) => {
          const allOnlines = [];
          for (let [_, socket] of this.server.of('/').sockets) {
      ...
      socket.on(
        ...
        async ({ content, to }: { content: string; to: string }) => {
          ...
          this.server.to(userId).emit('room', fromRoom);
          this.server.to(userId).emit('private received message', {
            message,
            room: to,
          });
        }
